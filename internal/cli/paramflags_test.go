...
func TestParseParamFlagsStaysPure(t *testing.T) {
	t.Setenv("STOAT_SECRET_DOCKER_AUTHKEY", "must-not-be-read-during-parse")
	a, err := Parse([]string{
		"create", "work", "--image", "alpine", "--set", "docker.user=dev",
		"--secret", "docker.authkey",
	})
	if err != nil {
		t.Fatal(err)
	}
	want := []ParamEdit{
		{Recipe: "docker", Param: "user", Value: "dev"},
		{Recipe: "docker", Param: "authkey", Secret: true},
	}
...
func TestParseUnsetParamFlag(t *testing.T) {
	a, err := Parse([]string{"update", "work", "--unset", "docker.user"})
	if err != nil {
		t.Fatal(err)
	}
	want := []ParamEdit{{Recipe: "docker", Param: "user", Unset: true}}
	if !reflect.DeepEqual(a.Params, want) {
...
func TestCLICreateAndUpdatePersistsParamEdits(t *testing.T) {
	...
	t.Setenv("STOAT_SECRET_DOCKER_AUTHKEY", secret)
	var out, errOut bytes.Buffer
	if code := Main([]string{
		"--json", "create", "work", "--image", "alpine-virt-3.24.1-x86_64.iso",
		"--recipes", "docker", "--set", "docker.user=alice", "--secret", "docker.authkey",
	...
	errOut.Reset()
	if code := Main([]string{
		"--json", "update", "work", "--set", "docker.user=bob", "--unset", "docker.authkey",
	}, "test", strings.NewReader(""), &out, &errOut); code != ExitOK {
