...
// Until is a state Wait can block for. It is deliberately a small, closed
...
// the state get ErrCannotReach; everything else is left to ctx.
...
//
// It exists separately from Start's own wait flag (StartOpts.Wait in the
// design doc) because a caller does not always control when the VM started
// starting: it may have been started by an earlier call, by another process,
// or found already mid-boot by List. Wait lets that caller block on the
// outcome without re-polling Get in a loop, which is the failure mode this
// is here to prevent.
...
// bannerReady makes; this is a ctx-aware reimplementation rather than a
// call to it, because sshx.Wait takes a fixed timeout and cannot be told to
// give up early when ctx is cancelled mid-dial.
...
// configured" is, and a caller may legitimately be about to retry the apply
// that just failed. That case is left to ctx rather than guessed at.
...
// pollUntil checks check immediately (so an already-satisfied condition
// returns with no delay and no ticker setup) and then polls every
