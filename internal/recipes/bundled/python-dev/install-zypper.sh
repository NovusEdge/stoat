...
# Leap 16 names the interpreter package for its version. python313 brings
# python3-base, which owns /usr/bin/python3 and the venv module; python313-pip
# owns /usr/bin/pip3. Verified on Leap 16.0.
stoat_pkg_setup
stoat_pkg_install python313 python313-pip
...
if [ -n "${STOAT_OUTPUT:-}" ]; then
    ...
    if [ -n "$tmp_root" ]; then
        ...
        printf 'venv=%s\n' "$venv_dir" >> "$STOAT_OUTPUT"
        printf 'venv_python=%s\n' "$venv_python" >> "$STOAT_OUTPUT"
    fi
fi
...
case "$root_fstype" in
tmpfs | overlay)
    echo "NOTE: this is a live VM (root is $root_fstype, in RAM). Everything installed above is gone after a reboot. Use a disk VM to keep it."
    ;;
*)
