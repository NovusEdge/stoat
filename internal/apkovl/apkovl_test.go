...
func TestBuildIncludesInstallStageForDiskMode(t *testing.T) {
	...
	if !strings.Contains(content["etc/stoat/answerfile"], "HOSTNAMEOPTS") {
		t.Error("etc/stoat/answerfile missing or not a setup-alpine answerfile")
	}
	script, ok := content["etc/local.d/stoat-install.start"]
	if !ok {
	...
	h, ok := hdrs["etc/runlevels/default/local"]
	if !ok || h.Typeflag != tar.TypeSymlink || h.Linkname != "/etc/init.d/local" {
		t.Error("missing etc/runlevels/default/local symlink, local.d would never run")
	}
}
