...
// not touch app.go, so a second modal kind there is out of scope here.
// The state/resize/esc-to-close shape still follows imagemodal.go's
...
type logPager struct {
	...
	// so the footer can say the pager is showing a tail rather than let the
	// user assume they're looking at the whole thing.
	truncated bool
	// scrolledToBottom becomes true once the initial scroll-to-the-end has
	// run. It has to wait for the first real resize: the viewport is built
	// at a placeholder 1x1 size (the terminal isn't known yet), and
	// GotoBottom at that width would compute against soft-wrapped lines of
	// one character each, an offset that overshoots once the real width
	// rewraps the same content into far fewer lines, rendering blank.
	scrolledToBottom bool
}

// consoleTailBytes bounds how much of console.log a pager load reads. A
// long-running VM's console log has no size limit, so tailReadCloser seeks
// near the end of it (when the reader supports seeking; see there) instead
// of reading the whole file into memory just to show the last screenful.
...
// startProvision), not in the key handler itself.
...
// handles a VM that has never started (empty reader, not an error) and a
// broken vm.toml (serves the directory's console.log regardless); neither
...
// there is no reason to hold all of it in memory to show the tail. Only the
// io.NopCloser-over-an-empty-reader case core.Logs returns for a VM with no
// console log yet falls through to the plain ReadAll below, and that one
// has nothing in it to make reading it whole a problem.
...
// panel over the screen rather than a full-bleed takeover, the same call
// imagemodal.go's width/height fractions make for the image picker.
const logPagerMargin = 4

// resize fits the pager to the terminal. Called every render (from
// viewDetail) rather than on tea.WindowSizeMsg: this file cannot add a
// message route in app.go, and SetWidth/SetHeight are no-ops on an
// unchanged value, so calling it unconditionally each frame is cheap.
