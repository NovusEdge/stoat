...
- **Disk** VMs boot from the same kind of ISO but keep a qcow2 disk that survives restarts. There is no apkovl built for disk mode, so the guest comes up with no ssh and no key installed — you install the OS yourself at the QEMU console (`setup-alpine`, for an Alpine disk image), then press `i` on the detail screen to mark the VM `installed`. Until you do, `s` and `p` refuse with a message pointing you at that same step.
