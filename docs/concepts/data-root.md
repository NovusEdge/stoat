...
├── recipes/                   # installed recipe scripts/fragments
├── logs/
...
- `isos/` holds both plain ISOs (Alpine) and downloaded cloud images
  ...
  `disk.qcow2` is a copy-on-write overlay backed by one of these — the base
...
- `ovl/` is reused for two unrelated purposes depending on mode: the Alpine
  ...
  `disk` VM has no `ovl/` contents at all — there's nothing to inject before
  you've installed the guest OS yourself.
...
| `share` | string | Host directory exposed to the guest as `/mnt/host`; empty means no share |
| `sshport` | int | The host-side loopback port forwarded to the guest's port 22 |
| `recipes` | []string | Filenames (from `recipes/`) selected for this VM |
...
it runs, but that install step never overwrites a file that's already
there — so local edits to a recipe survive a stoat upgrade.
