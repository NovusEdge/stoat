...
The TUI offers provisioning after SSH becomes reachable when a VM starts. A
live VM is offered on each start because its root filesystem is temporary. A
disk VM is offered while it needs provisioning. Cloud-init recipes run from
the seed at first boot; stoat discovers their marker files if a later `apply`
needs to reconcile state.
