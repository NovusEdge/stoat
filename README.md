...
- Ships a few ready-made recipes — XFCE, Docker, dev tools, Tailscale — to run post-boot over ssh or bake into a cloud-init seed.
- QEMU processes are tracked by pidfile, not supervised: `stoat` can exit and the VM keeps running.
- A TUI for interactive use, and a scriptable CLI (`ls`, `up`, `down`, `ssh`, `provision`, `rm`, `logs`, `doctor`) covering the same operations for scripts and automation.

## What it looks like

```
  ● alpine-live    live   2048M  2c  up 12m34s  :2200
❯ ○ ubuntu-dev     cloud  4096M  4c  —
  ✗ old-vm         broken: unexpected EOF

↵ start/stop  →/l details  s ssh  p provision  / search  n new  d delete  q quit  ? help
```
...
Grab the tarball and `checksums.txt` for your architecture from the [releases page](https://github.com/NovusEdge/stoat/releases).
...
The flake's `vendorHash` is currently a placeholder (`lib.fakeHash`), so the first `nix build` will fail and print the real hash in its error — paste that into `flake.nix` before it builds. `nix run .` and `devShells.default` (go, just, qemu, openssh) are also defined.
...
Global flags: `-q`/`--quiet`/`--no-interactive` suppress progress chatter (results and errors still print). Exit codes: 0 success, 1 runtime failure, 2 usage error. Full reference: [docs/reference/cli.md](docs/reference/cli.md).
